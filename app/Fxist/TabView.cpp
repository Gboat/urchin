...
int	CTabView::AddPage (LPCTSTR pszTitle, int nPageID, CRuntimeClass* pChildDialog,UINT nTemplateID)
{	
	TC_ITEM item;
	item.mask = TCIF_TEXT;
	...
	else 
	{
		...
		//
		CDialog* pDialog = (CDialog*)pChildDialog->CreateObject();
		...
		if (!pDialog->Create (nTemplateID, this)) 
		{
			GetTabCtrl ().DeleteItem (nIndex);
...
BOOL CTabView::RemovePage (int nIndex)
{
	...
	//
	CDialog* pDialog = m_pPages[nIndex];
	if (pDialog != NULL) 
	{
		pDialog->DestroyWindow ();	
		delete pDialog;
...
BOOL CTabView::ActivatePage (int nIndex)
{
	...
	if (nIndex != -1) {
		CDialog* pDialog = m_pPages[nOldIndex];
		if (pDialog != NULL) {
...
void CTabView::OnDestroy () 
{
	...
	//
	// Destroy dialogs and delete CDialog objects.
	//
	if (nCount > 0) {
		for (int i=nCount - 1; i>=0; i--) {
			OnDestroyPage (i, m_nPageIDs[i]);
			CDialog* pDialog = m_pPages[i];
			if (pDialog != NULL) {
				pDialog->DestroyWindow ();
				delete pDialog;
			}
