...
namespace firtex
{
	namespace utility
	{
		 ...
		 */
		DIR * opendir (const TCHAR *szPath)
		{
			DIR *nd;
//			unsigned int rc;
			_TCHAR szFullPath[FX_MAX_PATH];
			...
			_tfullpath (szFullPath, szPath, FX_MAX_PATH);
			...
			if (nd->dd_name[0] != _T('\0') &&
				...
				nd->dd_name[_tcslen (nd->dd_name) - 1] != _T('\\'))
			{
				_tcscat (nd->dd_name, DIRENT_SLASH);
			}

			/* Add on the search pattern */
			_tcscat (nd->dd_name, DIRENT_SEARCH_SUFFIX);
